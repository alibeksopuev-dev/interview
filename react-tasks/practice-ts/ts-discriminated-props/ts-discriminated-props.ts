...
assertEqual(
  renderButton({ label: 'Открыть', href: '/page', external: true }),
  { tag: 'a', label: 'Открыть', href: '/page', target: '_blank' },
  'рендер ссылки с external=true даёт target=_blank',
)

assertEqual(
  renderButton({ label: 'Открыть', href: '/page' }),
  { tag: 'a', label: 'Открыть', href: '/page', target: undefined },
  'рендер ссылки без external оставляет target undefined',
)

const onClick = () => {}
assertEqual(
  renderButton({ label: 'Сохранить', onClick }),
  { tag: 'button', label: 'Сохранить', onClick },
  'рендер кнопки даёт tag button и тот же onClick',
)
