...
exports.deepCloneAdvanced = void 0;
/**
...
function deepCloneAdvanced(value, cache = new Map()) {
    ...
    if (type === 'set') {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        value.forEach(item => cloned.add(deepCloneAdvanced(item, cache)));
        return cloned;
    }
    if (type === 'map') {
        const cloned = new Map();
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        value.forEach((val, key) => cloned.set(key, deepCloneAdvanced(val, cache)));
        return cloned;
    }
    if (type === 'array') {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return value.map(item => deepCloneAdvanced(item, cache));
    }
    if (type === 'date')
...
exports.deepCloneAdvanced = deepCloneAdvanced;
/**
