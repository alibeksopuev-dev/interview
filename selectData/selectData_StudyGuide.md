...
* **Решение:** Если перевернуть массив в самом начале (`sessions.slice().reverse()`), то **последнее вхождение пользователя станет первым встреченным**. Мы мержим все последующие вхождения в это «первое» встреченное. В конце мы переворачиваем результат обратно, и все элементы возвращаются на свои хронологически правильные места.

#### 💡 Пример разбора «на пальцах»
Пусть на входе массив сессий (слева направо — от старых к новым):
```ts
const sessions = [
  { user: 'Алибек', duration: 10 }, // Индекс 0
  { user: 'Иван',   duration: 20 }, // Индекс 1
  { user: 'Алибек', duration: 30 }  // Индекс 2 (последнее хронологическое вхождение Алибека)
]
```
При `merge: true` сессии Алибека объединятся (`duration: 40`), и он должен стоять **после** Ивана (на позиции своего последнего вхождения — индекс 2).

1. **Без реверса (обычный обход):**
   * Обрабатываем Алибека (10) $\rightarrow$ Создаем запись для Алибека.
   * Обрабатываем Ивана (20) $\rightarrow$ Создаем запись для Ивана (Алибек оказывается *перед* Иваном).
   * Слияние Алибека (30) происходит с первой записью $\rightarrow$ Итог: Алибек (40) стоит *перед* Иваном. **Это неверный порядок.**

2. **С реверсом:**
   * Разворачиваем: `[ Алибек(30), Иван(20), Алибек(10) ]`.
   * Первым видим Алибека (30) $\rightarrow$ Добавляем в результат и регистрируем в `Map`.
   * Вторым видим Ивана (20) $\rightarrow$ Добавляем в результат.
   * Третьим видим Алибека (10) $\rightarrow$ Находим его в `Map` и прибавляем к нему `10` (в массиве теперь лежит `[ Алибек(40), Иван(20) ]`).
   * Разворачиваем массив обратно: `[ Иван(20), Алибек(40) ]`. 

**Результат идеален:** Иван стоит первым, а объединенный Алибек вторым — на позиции своего последнего вхождения. И все это без сложного перемещения элементов в процессе.
