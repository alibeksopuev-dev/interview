...
exports.EventEmitterTraced = exports.EventEmitterFn = void 0;
// ─────────────────────────────────────────────────────────────────────────────
...
function EventEmitterFn() {
    this._events = Object.create(null);
}
exports.EventEmitterFn = EventEmitterFn;
EventEmitterFn.prototype.on = function (eventName, listener) {
    if (!Object.hasOwn(this._events, eventName)) {
        this._events[eventName] = [];
    }
    this._events[eventName].push(listener);
...
EventEmitterFn.prototype.off = function (eventName, listener) {
    if (!Object.hasOwn(this._events, eventName)) {
        return this;
    }
    const listeners = this._events[eventName];
...
EventEmitterFn.prototype.emit = function (eventName, ...args) {
    if (!Object.hasOwn(this._events, eventName) || this._events[eventName].length === 0) {
        return false;
    }
    const listeners = this._events[eventName].slice();
...
class EventEmitterTraced {
    ...
    off(eventName, listener) {
        ...
        this._events[eventName].splice(index, 1);
        this.log(`off('${eventName}', ${listener.name || '<anonymous>'}) удалил запись #${index} → корзина теперь ${this.snapshot(eventName)}`);
        return this;
    }
    emit(eventName, ...args) {
        if (!Object.hasOwn(this._events, eventName) || this._events[eventName].length === 0) {
...
exports.EventEmitterTraced = EventEmitterTraced;
...
async function realWorldExample() {
    ...
    const hadListeners = toastBus.emit('toast', { message: 'Это уже никто не увидит', type: 'info' });
...
function runTests() {
    ...
    emitter.on('foo', (a, b) => console.log(`Произведение: ${a * b}`));
